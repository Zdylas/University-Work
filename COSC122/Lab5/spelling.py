...
class ChainingHashTable():
    ...
    ***** DON'T worry you will fix this later:)
    ...
    def __contains__(self, item):
        ...
        list_value = self._hash(item)

        if item in self._data[list_value]:
            return True
        else:
            return False 
        # ===end student section===
...
class QuadraticHashTable():
    ...
    def store(self, item):
        ...
        first_hash = self._hash(item)
        try_number = 0
        if self._data[first_hash] == None:
            self._data[first_hash] = item
        else:
            better_slot = self._next_free_slot(first_hash)
            self._data[better_slot] = item
    ...
    def __contains__(self, item):
        ...
        if self._data[first_hash] == item:
            ...
            i = 1
            current_index = (first_hash + i) % self.n_slots
            while not found and i <= self.n_slots:
...
def spellcheck_with_list(document_word_list, dictionary_word_list, quiet_mode=False):
    ...
    for i in document_word_list:
        if i not in dictionary_word_list:
            print(i)
            num_errors += 1
            unique_errors.add(i)
    # ===end student section===
...
def spellcheck_with_hashtable(document_word_list,
    ...
    """
    hash_table, build_time = build_hash_table(ht_type, ht_size, dictionary_word_list)
...
if __name__ == '__main__':
    ...
    # Idividual tests:
    # doctest.run_docstring_examples(ChainingHashTable, None, verbose=with_verbose)
