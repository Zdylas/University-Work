...
def evaluate_postfix(expression):
    ...
    for token in tokens:
        if token not in OP_PREC:
            ...
            top = float(my_stack.pop())
            bottom = float(my_stack.pop())
            new_value = calculate(token, bottom, top)
            my_stack.push(new_value)
...
def infix_to_postfix(infix_expression):
    ...
    for token in tokens:
        if token not in OP_PREC:
            ...
            while my_stack.peek() != "(":
                output += f" {my_stack.pop()}"
            while my_stack.peek() == "(":
                my_stack.pop()    
        else:
            while my_stack.peek() in OP_PREC:
                if OP_PREC[my_stack.peek()] >= OP_PREC[token]:
                    output += f" {my_stack.pop()}"
                else: 
                    break
            my_stack.push(token)
